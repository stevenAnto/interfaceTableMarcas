...
window = tkinter.Tk()
window.title("Mantenimiento tabla GZZ_MARCA")
...
def enviarBD():
    ...
    if marFlaAct:
        ...
        if estadosBotonActualizar in acciones:
            if estadosBotonActualizar == "update" and estadoRegistroInput == "*":
                mostrarVentanaEmergente("No es posible modificar registros eliminados")
            else:
...
def llenarGrilla():
    ...
    conexion = back.establecer_conexion()
    registrosTodos = back.seleccionar(conexion,"GZZ_MARCA")
    contadorGrilla=0
...
frame = tkinter.LabelFrame(window, text="GZZ_MARCA")
