...
window = tkinter.Tk()
window.title("Mantenimiento tabla GZZ_ZONA")
...
def enviarBD():
    ...
    if marFlaAct:
        ...
        if estadosBotonActualizar=="insertar":
            ...
            nomTable = "GZZ_ZONA"
            atributosTabla = "(ZonCod, ZonDes, ZonEstReg)"
...
def llenarGrilla():
    ...
    conexion = back.establecer_conexion()
    registrosTodos = back.seleccionar(conexion,"GZZ_ZONA")
    contadorGrilla=0
...
frame = tkinter.LabelFrame(window, text="GZZ_ZONA")
#por buena practica este Fram contendra todos los widgets
frame.grid(row=0,column=0,sticky="nswe")

registro = tkinter.LabelFrame(frame,text="Registro de Zonas")
tabla = tkinter.LabelFrame(frame,text="Tabla de Zonas")
