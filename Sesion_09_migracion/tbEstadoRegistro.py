...
window = tkinter.Tk()
window.title("Mantenimiento tabla GZZ_Estado_Registro")
...
def enviarBD():
    ...
    if marFlaAct:
        ...
        if estadosBotonActualizar=="insertar":
            ...
            nomTable = "GZZ_ESTADO_REGISTRO"
            atributosTabla = "(EstRegCod, EstRegDes, EstRegEstReg)"
            inserto=back.insertar(conexion,nomTable,atributosTabla,codigoInput,
...
def llenarGrilla():
    ...
    conexion = back.establecer_conexion()
    registrosTodos = back.seleccionar(conexion,"GZZ_ESTADO_REGISTRO")
    contadorGrilla=0
...
frame = tkinter.LabelFrame(window, text="GZZ_ESTADO_REGISTRO")
#por buena practica este Fram contendra todos los widgets
frame.grid(row=0,column=0,sticky="nswe")

registro = tkinter.LabelFrame(frame,text="Registro de Estados")
tabla = tkinter.LabelFrame(frame,text="Tabla_de_Estados_de_Registro")
